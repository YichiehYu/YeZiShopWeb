...
public class HttpURLConnectionUtils {
	 ...
	 * 接口调用 POST
	 */
	public static String httpURLConnectionPOST(String httpUrl, Map<String, String> parmMap) {
		try {
			...
			for (String key : parmMap.keySet()) {
				parm += key;
				parm += "=";
				parm += URLEncoder.encode(parmMap.get(key), "utf-8");// URLEncoder.encode()方法 为字符串进行编码
				parm += "&";
			}
			if(!"".equals(parm)) {
